# Code adapted from https://raw.githubusercontent.com/iwyoo/LSTM-autoencoder/master/LSTMAutoencoder.py
...
class LSTMAutoencoder(object):
  ...
  """
    ...
    """
        ...
        self.batch_num = inputs.get_shape()[0]
        self.elem_num = inputs.get_shape()[2]

        #self._enc_cell = LSTMCell(hidden_num)
        self._enc_cell = cell(self.lstm_statesize, activation=tf.nn.elu)
        self._dec_cell = cell(self.lstm_statesize, activation=tf.nn.elu)
        ...
        with tf.variable_scope('decoder') as vs:
            ...
            if decode_without_input:
                dec_inputs = tf.zeros(tf.shape(inputs), dtype=tf.float32)
                ...
                dec_input_ = tf.zeros(tf.shape(inputs),
                        dtype=tf.float32)
