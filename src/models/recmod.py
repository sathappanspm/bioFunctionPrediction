...
class GORNNDecoder(object):
    ...
    def build(self):
        ...
        log.info('actions-{}, predictions-{}'.format(predlabels.get_shape(),  actionpreds.get_shape()))
        #ipdb.set_trace()
        actionmask = tf.reshape(tf.cast(tf.logical_not(tf.equal(self.ys_, 0)), dtype=tf.float32), shape=[-1, 1])

        actionerr = tf.reduce_mean((actionmask - actionpreds)**2)
