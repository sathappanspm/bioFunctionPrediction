...
class ConvAutoEncoder(object):
    ...
    def build_train (self):
        #_x = tf.contrib.layers.flatten(self.x)
        #_y = tf.contrib.layers.flatten(self.final_op)

        # self.loss = tf.losses.mean_squared_error(_x,_y)
        #self.loss = tf.losses.softmax_cross_entropy(onehot_labels=_x,logits=_y)
        self.loss = tf.losses.softmax_cross_entropy(onehot_labels=self.x,logits=self.final_op)
